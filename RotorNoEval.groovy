...
class Block {
  ...
  def stack;
  def wheels;
  public Block(String c) { code = c; stack = RotorNoEval.stack; wheels = RotorNoEval.wheels;}

  void parse() {
    for(;codePointer < code.length();codePointer++) {
      char instruction = code[codePointer];
      if(instruction == '<') {
        if(RotorNoEval.wheelPointer == 0) RotorNoEval.wheelPointer = wheels.length-1;
        else RotorNoEval.wheelPointer--;
      } else if(instruction == '>') {
        if(RotorNoEval.wheelPointer == wheels.length-1) RotorNoEval.wheelPointer = 0;
        else RotorNoEval.wheelPointer++;
      } else if(instruction == '`') {
        String literal = "";
        ...
        def i = stack.pop();
        if(i >= 0 && i < wheels.length) RotorNoEval.wheelPointer = i;
      } else if(instruction == '@') {
        stack.push(stack[-3]);
        ...
        RotorNoEval.reg = stack[-1];
      } else if(instruction == '~') {
        stack.push(RotorNoEval.reg);
      } else if(instruction == '{') {
        String literal = "";
        for(codePointer++; codePointer < code.length(); codePointer++) {
  ...
  void redefine(char c, int n) {
    int index1 = Arrays.binarySearch(wheels[RotorNoEval.wheelPointer],(String[])[c as String],comp);
    int index2 = Arrays.binarySearch(wheels[n],(String[])[c as String],comp);
    if(index1 >= 0 && index2 >= 0) {
      wheels[RotorNoEval.wheelPointer][index1][1] = wheels[n][index2][1];
    }
  }

  void parseInstruction(char c) {
    int index = Arrays.binarySearch(wheels[RotorNoEval.wheelPointer],(String[])[c as String],comp);
    if(index >= 0) Eval.xy(stack,RotorNoEval.reg,wheels[RotorNoEval.wheelPointer][index][1]);
  }
